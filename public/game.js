...
$('mapBtn').onclick=()=>{$('mapModal').classList.remove('hidden');mapOpen=true;requestAnimationFrame(()=>{resize();drawBigMap()})};$('closeMap').onclick=()=>{$('mapModal').classList.add('hidden');mapOpen=false};$('emergencyBtn').style.display='none';
...
function renderContext(){if(!me||!me.alive){$('contextButtons').innerHTML='';return}const camx=Math.max(0,Math.min(W-innerWidth,me.x-innerWidth/2)),camy=Math.max(0,Math.min(H-innerHeight,me.y-innerHeight/2));$('contextButtons').innerHTML='';const task=near(T.filter(t=>!tasksDone.includes(t.id)),120);if(task&&task.d<125){
 ...
 addBtn('🔧 انجام مأموریت','taskAction',p.x+18,p.y-48,()=>startMission(task));
}
...
function wiresMission(t){const colors=['قرمز','آبی','زرد','سبز','بنفش'],cs=['#ef476f','#118ab2','#ffd166','#06d6a0','#9b5de5'];let paired=0,selected=null,dragging=null;const e=missionOverlay('⚡ سیم‌کشی برق','<p>روش گوشی: اول یک سیم را لمس کن، بعد سیم هم‌رنگ را لمس کن. با کشیدن هم می‌توانی وصل کنی.</p><div id="wireBoard" class="wireBoard"></div>');const b=$('wireBoard');let left=[],right=[];colors.forEach((c,i)=>{left.push(i);right.push(i)});left.sort(()=>Math.random()-.5);right.sort(()=>Math.random()-.5);function render(){b.innerHTML='<div class="wireCol" id="wl"></div><div class="wireCol" id="wr"></div>';left.forEach(i=>add(i,'wl'));right.forEach(i=>add(i,'wr'))}function finish(a){if(!a||a.classList.contains('matched'))return;const i=+a.dataset.i;const nodes=b.querySelectorAll('.wireNode');nodes.forEach(x=>{if(+x.dataset.i===i)x.classList.add('matched')});paired++;selected=null;dragging=null;if(paired===5)setTimeout(()=>doneTask(t.id),300)}function add(i,id){const d=document.createElement('button');d.type='button';d.className='wireNode';d.dataset.i=i;d.dataset.side=id;d.style.borderColor=cs[i];d.style.color=cs[i];d.textContent='● '+colors[i];d.onpointerdown=ev=>{if(d.classList.contains('matched'))return;selected=d;dragging={i,from:id};d.classList.add('selected');try{d.setPointerCapture(ev.pointerId)}catch(e){}};d.onpointerup=ev=>{if(!dragging)return;const target=d;if(target!==dragging.el&&id!==dragging.from&&+target.dataset.i===dragging.i)finish(target);b.querySelectorAll('.wireNode').forEach(x=>x.classList.remove('selected'));dragging=null};d.onclick=()=>{if(d.classList.contains('matched'))return;if(!selected){selected=d;d.classList.add('selected');return}if(selected!==d&&selected.dataset.side!==d.dataset.side&&selected.dataset.i===d.dataset.i){finish(d)}else{b.querySelectorAll('.wireNode').forEach(x=>x.classList.remove('selected'));selected=d;d.classList.add('selected')}};document.getElementById(id).appendChild(d)}render()}
...
function drawBigMap(){
 ...
 if(bigMap.width!==Math.floor(bigMap.clientWidth*dpr)||bigMap.height!==Math.floor(bigMap.clientHeight*dpr)){bigMap.width=Math.floor(bigMap.clientWidth*dpr);bigMap.height=Math.floor(bigMap.clientHeight*dpr);bctx.setTransform(dpr,0,0,dpr,0,0)}
