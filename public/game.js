...
addEventListener('keydown',e=>{const k=e.key.toLowerCase();keys[k]=true;if(e.repeat)return;if(!me||!me.alive||meeting)return;if(k==='z'){const task=near(T.filter(t=>!tasksDone.includes(t.id)),125);if(task&&task.d<125){showMissionInfo(task);e.preventDefault()}}else if(k==='x'){if(role==='infiltrator'){const target=near(players.filter(p=>p.id!==myId&&p.alive),60);if(target&&target.d<=55){socket.emit('kill',{targetId:target.id});e.preventDefault()}}}else if(k==='c'){const body=near(players.filter(p=>!p.alive),85);if(body&&body.d<75){socket.emit('report');e.preventDefault()}else if(Math.hypot(me.x-table.x,me.y-table.y)<115){socket.emit('emergency');e.preventDefault()}}});addEventListener('keyup',e=>keys[e.key.toLowerCase()]=false);function move(){if(!me)return;let x=0,y=0;if(keys.w||keys.arrowup)y--;if(keys.s||keys.arrowdown)y++;if(keys.a||keys.arrowleft)x--;if(keys.d||keys.arrowright)x++;x+=joy.x;y+=joy.y;let l=Math.hypot(x,y);if(l)socket.emit('move',{dx:x/Math.max(1,l),dy:y/Math.max(1,l)})}setInterval(move,100);
