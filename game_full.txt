...
const ART_Y=Array.from({length:20},(_,i)=>i*ART_CELL);
const ART_COLOR_INDEX=new Map(FREE_COLORS.map(([,c],i)=>[c.toLowerCase(),i]));
const PALETTE_VERSION='v19';
if(localStorage.getItem('skinPaletteVersion')!==PALETTE_VERSION){localStorage.setItem('skinPaletteVersion',PALETTE_VERSION);localStorage.setItem('skinColor',FREE_COLORS[0][1]);}
...
mines:["حدس مین","از ۱۰ دایره، دقیقاً ۲ دایره مین هستند. هر دو مین را پیدا کن.","اگر روی دایره اشتباه بزنی مرحله دوباره شروع می‌شود."],
data:["دریافت فایل","اول کارت دارای صورت خودت را دو بار چپ و راست بکش تا دستگاه شناسایی‌ات کند. سپس فایل را دریافت کن.","نوار دریافت از ۰ تا ۱۰۰ در ۱۰ ثانیه پر می‌شود."],
engine:["تعمیر موتور","۴ قطعه خراب را در جای درست موتور قرار بده.","قطعه را انتخاب کن و جای خالی هم‌شکل خودش را بزن."],
cargo:["مرتب‌سازی محموله","جعبه‌ها را بر اساس رنگ یا شماره در محل مناسب قرار بده.","هر جعبه را انتخاب کن و قفسه هم‌رنگ/هم‌شماره را بزن."],
mapPuzzle:["بازسازی نقشه سفینه","۸ تکه نقشه را مثل پازل کنار هم قرار بده.","هر قطعه را انتخاب کن و خانه مناسب خودش را بزن."],
code:["وارد کردن رمز","۴ سرنخ داخل اتاق را پیدا کن و رمز ۴ رقمی را وارد کن.","هر سرنخ یک رقم از رمز را نشان می‌دهد؛ ترتیب رقم‌ها از ۱ تا ۴ است."],
panel:["تعمیر پنل","۴ قطعه خراب را پیدا و جایگزین کن.","هر قطعه را انتخاب کن و محل هم‌شکلش را بزن."],
gears:["چرخ‌دنده‌ها","چرخ‌دنده‌ها را در جای مناسب بگذار.","هر چرخ‌دنده را انتخاب کن و جای هم‌شکل و هم‌اندازه‌اش را بزن."]};
...
function startMission(t){
 ...
 if(t.kind==='engine')return engineMission(t);
 if(t.kind==='cargo')return cargoMission(t);
 if(t.kind==='mapPuzzle')return mapPuzzleMission(t);
 if(t.kind==='code')return codeMission(t);
 if(t.kind==='panel')return panelMission(t);
 if(t.kind==='gears')return gearsMission(t);
...
function makeMatchMission(title,desc,items,slotTitle,done){
 let selected=null,placed=new Set();
 const e=missionOverlay(title,'<p>'+desc+'</p><div class="matchBoard"><div class="matchItems" id="matchItems"></div><div class="matchSlots" id="matchSlots"></div></div><div id="matchMsg" class="missionMsg"></div>');
 const itemsBox=$('matchItems'),slotsBox=$('matchSlots');
 [...items].sort(()=>Math.random()-.5).forEach(item=>{const b=document.createElement('button');b.className='matchItem';b.textContent=item.icon+' '+item.label;b.dataset.id=item.id;b.onclick=()=>{if(placed.has(item.id))return;document.querySelectorAll('.matchItem').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');selected=item.id};itemsBox.appendChild(b)});
 [...items].sort(()=>Math.random()-.5).forEach(item=>{const b=document.createElement('button');b.className='matchSlot';b.dataset.id=item.id;b.innerHTML='<span>'+slotTitle+'</span><b>'+item.target+'</b>';b.onclick=()=>{if(selected===null)return;if(String(selected)===String(item.id)){placed.add(item.id);b.classList.add('filled');b.innerHTML='✅ <b>'+item.target+'</b>';const src=[...itemsBox.children].find(x=>x.dataset.id===String(item.id));if(src)src.classList.add('placed');selected=null;document.querySelectorAll('.matchItem').forEach(x=>x.classList.remove('selected'));if(placed.size===items.length){$('matchMsg').textContent='✅ انجام شد';setTimeout(done,350)}}else{$('matchMsg').textContent='❌ این قطعه اینجا نیست.'}};slotsBox.appendChild(b)});
 return e;
}
function engineMission(t){const parts=[{id:1,icon:'🔩',label:'پیچ',target:'جای پیچ'},{id:2,icon:'🔧',label:'آچار',target:'جای آچار'},{id:3,icon:'⚡',label:'فیوز',target:'جای فیوز'},{id:4,icon:'🧯',label:'مخزن',target:'جای مخزن'}];makeMatchMission('🛠️ تعمیر موتور','۴ قطعه خراب را در جای درست موتور قرار بده.',parts,'جایگاه',()=>doneTask(t.id));}
function cargoMission(t){const parts=[{id:1,icon:'🟥',label:'قرمز 1',target:'قرمز 1'},{id:2,icon:'🟦',label:'آبی 2',target:'آبی 2'},{id:3,icon:'🟨',label:'زرد 3',target:'زرد 3'},{id:4,icon:'🟥',label:'قرمز 4',target:'قرمز 4'},{id:5,icon:'🟦',label:'آبی 5',target:'آبی 5'},{id:6,icon:'🟨',label:'زرد 6',target:'زرد 6'}];makeMatchMission('📦 مرتب‌سازی محموله','جعبه‌ها را بر اساس رنگ یا شماره در محل مناسب قرار بده.',parts,'قفسه',()=>doneTask(t.id));}
function mapPuzzleMission(t){const parts=[{id:1,icon:'🗺️',label:'تکه 1',target:'بالا-چپ'},{id:2,icon:'🗺️',label:'تکه 2',target:'بالا-میانه'},{id:3,icon:'🗺️',label:'تکه 3',target:'بالا-راست'},{id:4,icon:'🗺️',label:'تکه 4',target:'میانه-چپ'},{id:5,icon:'🗺️',label:'تکه 5',target:'میانه-راست'},{id:6,icon:'🗺️',label:'تکه 6',target:'پایین-چپ'},{id:7,icon:'🗺️',label:'تکه 7',target:'پایین-میانه'},{id:8,icon:'🗺️',label:'تکه 8',target:'پایین-راست'}];makeMatchMission('🧩 بازسازی نقشه سفینه','۸ تکه نقشه را مثل پازل کنار هم قرار بده.',parts,'خانه',()=>doneTask(t.id));}
function codeMission(t){const code=Array.from({length:4},()=>Math.floor(Math.random()*10));let entered='';const clues=code.map((n,i)=>'<div class="codeClue"><b>🔴 سرنخ '+(i+1)+'</b><span>رقم '+(i+1)+' = '+n+'</span></div>').join('');missionOverlay('🔢 وارد کردن رمز','<p>۴ سرنخ قرمز داخل اتاق را بخوان و رمز را وارد کن.</p><div class="clueGrid">'+clues+'</div><div id="codeDisplay" class="codeDisplay">_ _ _ _</div><div class="keypad" id="keypad"></div><div id="codeMsg" class="missionMsg"></div>');const display=$('codeDisplay'),msg=$('codeMsg'),pad=$('keypad');const render=()=>display.textContent=(entered+'____').slice(0,4).split('').join(' ');for(let i=1;i<=9;i++){const b=document.createElement('button');b.textContent=i;b.onclick=()=>{if(entered.length<4){entered+=i;render()}};pad.appendChild(b)}const z=document.createElement('button');z.textContent='0';z.onclick=()=>{if(entered.length<4){entered+='0';render()}};pad.appendChild(z);const del=document.createElement('button');del.textContent='⌫';del.onclick=()=>{entered=entered.slice(0,-1);render()};pad.appendChild(del);const ok=document.createElement('button');ok.textContent='✅ تأیید';ok.className='confirmCode';ok.onclick=()=>{if(entered.length!==4){msg.textContent='۴ رقم را کامل وارد کن.';return}if(entered===code.join('')){msg.textContent='✅ رمز درست است';setTimeout(()=>doneTask(t.id),400)}else{msg.textContent='❌ رمز اشتباه است؛ سرنخ‌ها را دوباره بخوان.';entered='';render()}};pad.appendChild(ok);render();}
function panelMission(t){const parts=[{id:1,icon:'🔴',label:'چراغ',target:'قرمز'},{id:2,icon:'🟦',label:'نمایشگر',target:'آبی'},{id:3,icon:'🔋',label:'باتری',target:'باتری'},{id:4,icon:'🧰',label:'ابزار',target:'ابزار'}];makeMatchMission('🔧 تعمیر پنل','۴ قطعه خراب را پیدا و جایگزین کن.',parts,'محل',()=>doneTask(t.id));}
function gearsMission(t){const parts=[{id:1,icon:'⚙️',label:'کوچک',target:'جای چرخ کوچک'},{id:2,icon:'⚙️',label:'متوسط',target:'جای چرخ متوسط'},{id:3,icon:'⚙️',label:'بزرگ',target:'جای چرخ بزرگ'},{id:4,icon:'🛞',label:'دندانه‌دار',target:'جای چرخ دندانه‌دار'}];makeMatchMission('⚙️ چرخ‌دنده‌ها','چرخ‌دنده‌ها را در جای مناسب بگذار.',parts,'جایگاه',()=>doneTask(t.id));}
...
function drawPlayer(p){
  ...
  if(p.corpse){
    ...
    ctx.fillStyle=p.color||'#fb041c';ctx.beginPath();ctx.arc(-8,-3,12,0,Math.PI*2);ctx.fill();ctx.stroke();
  ...
  // هر بازیکن از خانه دقیق خودش در Sprite Sheet می‌آید؛ رنگ و اسکین با index عددی هماهنگ‌اند.
  const colorIndex=Number.isInteger(p.colorIndex)?Math.max(0,Math.min(19,p.colorIndex)):
    Math.max(0,ART_COLOR_INDEX.get(String(p.color||'').toLowerCase())??0);
  const shapeIndex=Number.isInteger(p.shapeIndex)?Math.max(0,Math.min(10,p.shapeIndex)):
    Math.max(0,FREE_SKINS.findIndex(x=>x[1]===(p.shape||'classic')));
  const sx=shapeIndex*ART_CELL, sy=colorIndex*ART_CELL;

  // سایه نرم زیر شخصیت تا رنگ‌های روشن (سفید/کرم/یاسی) هم کاملاً خوانا بمانند.
  ctx.fillStyle='rgba(0,0,0,.28)';
  ctx.beginPath();ctx.ellipse(0,31,29,8,0,0,Math.PI*2);ctx.fill();
  ...
  if(CHARACTER_ART.complete&&CHARACTER_ART.naturalWidth){
    ...
    ctx.drawImage(CHARACTER_ART,sx,sy,ART_CELL,ART_CELL,-46,-46,92,92);
    ctx.restore();
  }else{
    ctx.fillStyle=p.color||FREE_COLORS[colorIndex][1];ctx.beginPath();ctx.roundRect(-22,-26,44,52,18);ctx.fill();
  }

  ctx.fillStyle='#fff';ctx.font='14px Tahoma';ctx.textAlign='center';ctx.fillText(p.name,0,-54);
  if(performance.now()<roleTimer&&p.id===myId){
    ctx.fillStyle=role==='infiltrator'?'#d92742':'#267ee8';ctx.font='bold 22px Tahoma';
    ctx.fillText(role==='infiltrator'?'خائن':'خدمه',0,-75);
  }
  if(role==='infiltrator'&&partners.includes(p.name)&&p.id!==myId){
    ctx.fillStyle='#e32643';ctx.font='bold 14px Tahoma';ctx.fillText(p.name,0,-68)
  }
  ctx.restore();
}
function drawBigMap(){
 ...
 bctx.setTransform(dpr,0,0,dpr,0,0);bctx.clearRect(0,0,w,h);bctx.fillStyle='#020914';bctx.fillRect(0,0,w,h);
 if(MAP_ART.complete&&MAP_ART.naturalWidth){const scale=Math.min(w/MAP_NATIVE_W,h/MAP_NATIVE_H),mw=MAP_NATIVE_W*scale,mh=MAP_NATIVE_H*scale,mx=(w-mw)/2,my=(h-mh)/2;bctx.imageSmoothingEnabled=true;bctx.imageSmoothingQuality='high';bctx.drawImage(MAP_ART,mx,my,mw,mh);
  const sx=mw/W,sy=mh/H,ox=mx/sx,oy=my/sy;
  T.forEach(t=>{if(!tasksDone.includes(t.id)){const pulse=8+Math.sin(performance.now()/180)*2;const tx=(t.x*sx)+mx,ty=(t.y*sy)+my;bctx.fillStyle='#e63950';bctx.beginPath();bctx.arc(tx,ty,pulse+3,0,Math.PI*2);bctx.fill();bctx.fillStyle='#fff';bctx.font='bold 9px Arial';bctx.textAlign='center';bctx.fillText('!',tx,ty+3)}});
  if(me){const px=me.x*sx+mx,py=me.y*sy+my;bctx.fillStyle=me.color||'#fb041c';bctx.beginPath();bctx.arc(px,py,8,0,Math.PI*2);bctx.fill();bctx.strokeStyle='#102331';bctx.lineWidth=2;bctx.stroke()}
 }
}
