...
const SPRITE_NAMES={classic:'square',square:'square',diamond:'diamond',hex:'hex',cat:'cat',robot:'robot',ghost:'ghost',ninja:'ninja',star:'star',crown:'crown',minecraft:'minecraft'};
const COLOR_KEYS={'#ff5f6d':'red','#4fc3f7':'blue','#ffd166':'yellow','#55d68a':'green','#a78bfa':'purple','#ff9f43':'orange','#4d96ff':'deepblue','#ff7f50':'coral','#00c2a8':'teal','#e85dff':'pink'};
const SPRITES={};
Object.values(SPRITE_NAMES).forEach(n=>{SPRITES[n]={base:new Image()};SPRITES[n].base.src='/assets/characters/'+n+'.png';Object.values(COLOR_KEYS).forEach(k=>{SPRITES[n][k]=new Image();SPRITES[n][k].src='/assets/characters/'+n+'_'+k+'.png'})});
function getSprite(sh,color){const n=SPRITE_NAMES[sh]||'square';const key=COLOR_KEYS[color]||'red';return SPRITES[n]?.[key]||SPRITES[n]?.base}
function drawSprite(img,maxDim=82){if(!img||!img.naturalWidth)return false;const scale=Math.min(maxDim/img.naturalWidth,maxDim/img.naturalHeight);const w=img.naturalWidth*scale,h=img.naturalHeight*scale;ctx.drawImage(img,-w/2,-h/2,w,h);return h}
function drawPlayer(p){
  if(!p.alive && !p.corpse)return;
  ctx.save();ctx.translate(p.x,p.y);
  if(p.corpse){
    ctx.rotate(-0.18);
    const corpseImg=getSprite(p.shape||'classic',p.color);
    if(corpseImg&&corpseImg.complete)drawSprite(corpseImg,58);
    else{ctx.fillStyle='#8b3f4b';ctx.beginPath();ctx.ellipse(0,10,25,13,0,0,Math.PI*2);ctx.fill()}
    ctx.fillStyle='#fff';ctx.font='13px Tahoma';ctx.textAlign='center';ctx.fillText(p.name,0,38);
    ctx.restore();return;
  }
  const sh=p.shape||'classic';
  const img=getSprite(sh,p.color);
  const maxDim=(sh==='robot'||sh==='ghost'||sh==='crown'||sh==='minecraft')?88:82;
  const h=(img&&img.complete&&img.naturalWidth)?drawSprite(img,maxDim):0;
  const labelY=-(h?h/2:30)-8;
  ctx.fillStyle='#fff';ctx.font='14px Tahoma';ctx.textAlign='center';ctx.strokeStyle='rgba(0,0,0,.45)';ctx.lineWidth=3;ctx.strokeText(p.name,0,labelY);ctx.fillText(p.name,0,labelY);
  if(performance.now()<roleTimer&&p.id===myId){ctx.fillStyle=role==='infiltrator'?'#d92742':'#267ee8';ctx.font='bold 22px Tahoma';ctx.strokeText(role==='infiltrator'?'خائن':'خدمه',0,labelY-20);ctx.fillText(role==='infiltrator'?'خائن':'خدمه',0,labelY-20)}
  if(role==='infiltrator'&&partners.includes(p.name)&&p.id!==myId){ctx.fillStyle='#e32643';ctx.font='bold 14px Tahoma';ctx.fillText(p.name,0,labelY-18)}
  ctx.restore();
}
function drawBigMap(){
