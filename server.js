...
socket.on('startGame',()=>{const r=rooms.get(socket.data.code);if(!r){socket.emit('startResult',{ok:false,message:'اتاق پیدا نشد.'});return;}if(r.host!==socket.id){socket.emit('startResult',{ok:false,message:'فقط سازنده اتاق می‌تواند بازی را شروع کند.'});return;}if(r.started){socket.emit('startResult',{ok:true});return;}if(r.players.size<2){socket.emit('startResult',{ok:false,message:'برای شروع بازی حداقل ۲ بازیکن لازم است.'});return;}const ps=[...r.players.values()];const n=ps.length>=7?2:1;ps.forEach(p=>{p.role='crew';p.alive=true;p.lastKill=0;p.lastSab=0});for(let i=0;i<n;i++){const candidates=ps.filter(p=>p.role==='crew');const chosen=candidates[Math.floor(Math.random()*candidates.length)];chosen.role='infiltrator';}r.started=true;r.ended=false;r.winner=null;r.tasksDone.clear();r.sabotages=[];const tr=ps.filter(p=>p.role==='infiltrator');[...r.players.values()].forEach(p=>io.to(p.id).emit('roleSecret',{role:p.role,partners:p.role==='infiltrator'?tr.filter(q=>q.id!==p.id).map(q=>q.name):[]}));io.to(r.code).emit('gameStarted');socket.emit('startResult',{ok:true});send(r.code)});
