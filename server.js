...
socket.on('startGame',()=>{
 ...
 if(!r){socket.emit('errorMsg','اتاق پیدا نشد.');return;}
 if(r.host!==socket.id){socket.emit('errorMsg','فقط سازنده اتاق می‌تواند بازی را شروع کند.');return;}
 if(r.players.size<2){socket.emit('errorMsg','برای شروع بازی حداقل ۲ بازیکن لازم است.');return;}
 const ps=[...r.players.values()];
 const n=ps.length>=7?2:1;
 ps.forEach(p=>{p.role='crew';p.alive=true;p.lastKill=0;p.lastSab=0});
 for(let i=0;i<n;i++){
   const candidates=ps.filter(p=>p.role==='crew');
   const chosen=candidates[Math.floor(Math.random()*candidates.length)];
   chosen.role='infiltrator';
 }
 r.started=true;r.ended=false;r.winner=null;r.tasksDone.clear();r.sabotages=[];r.meeting=null;
 const tr=ps.filter(p=>p.role==='infiltrator');
 ps.forEach(p=>io.to(p.id).emit('roleSecret',{role:p.role,partners:p.role==='infiltrator'?tr.filter(q=>q.id!==p.id).map(q=>q.name):[]}));
 send(code);
 io.to(code).emit('gameStarted');
 io.to(code).emit('startGameOk');
});
