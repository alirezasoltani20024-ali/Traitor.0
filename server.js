...
socket.on('createRoom',d=>{const c=code();const r={code:c,host:socket.id,started:false,ended:false,winner:null,players:new Map(),tasksDone:new Set(),sabotages:[],meeting:null};r.players.set(socket.id,{id:socket.id,name:String(d?.name||'بازیکن').trim().slice(0,18)||'بازیکن',role:'crew',alive:true,x:1200,y:800,color:colors[0],shape:'classic',claimedRole:null,lastKill:0,lastSab:0});rooms.set(c,r);socket.join(c);socket.data.code=c;socket.emit('roomCreated',{code:c});send(c)});
socket.on('joinRoom',d=>{const c=String(d?.code||'').trim().toUpperCase(),r=rooms.get(c);if(!r)return socket.emit('errorMsg','کد اتاق اشتباه است.');if(r.started)return socket.emit('errorMsg','بازی شروع شده است.');if(r.players.size>=10)return socket.emit('errorMsg','اتاق پر است.');const i=r.players.size;r.players.set(socket.id,{id:socket.id,name:String(d?.name||'بازیکن').trim().slice(0,18)||'بازیکن',role:'crew',alive:true,x:1200+(Math.random()*120-60),y:800+(Math.random()*120-60),color:colors[i],shape:['classic','square','diamond','hex','cat','robot','ghost','ninja','star','crown'][i]||'classic',claimedRole:null,lastKill:0,lastSab:0});socket.join(c);socket.data.code=c;socket.emit('joined',{code:c});send(c)});
socket.on('setSkin',d=>{const r=rooms.get(socket.data.code),p=r?.players.get(socket.id);if(!p||r.started)return;const c=String(d?.color||''),sh=String(d?.shape||'classic');const allowed=['classic','square','diamond','hex','cat','robot','ghost','ninja','star','crown','minecraft'];if(colors.includes(c))p.color=c;if(allowed.includes(sh))p.shape=sh;send(r.code)});
socket.on('startGame',()=>{const r=rooms.get(socket.data.code);if(!r||r.host!==socket.id||r.players.size<3)return;const ps=[...r.players.values()],n=ps.length>=7?2:1;ps.forEach(p=>{p.role='crew';p.alive=true;p.lastKill=0;p.lastSab=0});for(let i=0;i<n;i++)ps.splice(Math.floor(Math.random()*ps.length),1)[0].role='infiltrator';r.started=true;r.ended=false;r.winner=null;r.tasksDone.clear();r.sabotages=[];const tr=ps.filter(p=>p.role==='infiltrator');[...r.players.values()].forEach(p=>io.to(p.id).emit('roleSecret',{role:p.role,partners:p.role==='infiltrator'?tr.filter(q=>q.id!==p.id).map(q=>q.name):[]}));io.to(r.code).emit('gameStarted');send(r.code)});
socket.on('move',d=>{const r=rooms.get(socket.data.code),p=r?.players.get(socket.id);if(!p||!p.alive||!r.started||r.ended||r.meeting)return;let dx=+d?.dx||0,dy=+d?.dy||0,l=Math.hypot(dx,dy);if(l>1){dx/=l;dy/=l}const nx=Math.max(80,Math.min(2320,p.x+dx*5)),ny=Math.max(80,Math.min(1520,p.y+dy*5));if(!hit(nx,p.y))p.x=nx;if(!hit(p.x,ny))p.y=ny;send(r.code)});
...
socket.on('report',()=>{const r=rooms.get(socket.data.code),p=r?.players.get(socket.id);if(!p?.alive||!r?.started||r.ended)return;const body=[...r.players.values()].find(q=>!q.alive&&dist(p,q)<75);if(!body)return;const bodyId=body.id;r.players.delete(bodyId);if(r.host===bodyId){r.host=[...r.players.keys()][0]||null}const deadSocket=io.sockets.sockets.get(bodyId);if(deadSocket)deadSocket.emit('removedAfterReport');startMeeting(r,p.name,'گزارش جسد');send(r.code);if(deadSocket)deadSocket.disconnect(true)});
